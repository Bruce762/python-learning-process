...
hands = mpHands.Hands(min_tracking_confidence=0.3, max_num_hands=1, min_detection_confidence=0.3, model_complexity=0 )#Hands是其中一個函式
    ...
    if ret:
        ...
        if result.multi_hand_landmarks:
            for handLms in result.multi_hand_landmarks:#放入偵測到的每一隻手
                #print(result.multi_hand_landmarks[0][0])
                #print(type(handLms))
                print(handLms)
                ...
                for i, lm in enumerate(handLms.landmark):
                    ...
                    if i==4:
                        cv2.circle(img, (xPos, yPos), 25, (255, 0, 0), FILLED)
                    #print(i, xPos, yPos)
    cTime =  time.time()
    fps = 1/(cTime-pTime)
    pTime = cTime
